...
exports.findAll = async(req, res) => {
  ...
  var supply = await Suppliers.find({ status : { $ne : 'Trash'}, $or: [{name: { $regex: new RegExp(search), $options: "i" }
	}]});
		const info2 = [];
		supply.forEach(function (doc, err) {
		  info2.push(doc._id);
		});
  var condition = { $or: [{ orderno: { $regex: new RegExp(search), $options: "i" }}, { job: { $regex: new RegExp(search), $options: "i" }}, { modifiedBy: { $in: info1 } }, { createdBy: { $in: info1 } }, { supplier: { $in: info2 } } ]};
  }
  else
  condition = {};
  ...
  Table.paginate(condition, { collation: { locale: "en" }, populate: ['createdBy', 'modifiedBy', 'supplier'], offset, limit, sort: sortObject })
    .then((data) => {
      res.send({
