  ...
  Table.findOne({ $or: [{ email: req.body.email}], status: { $ne:'Trash' } })
    .then((data) => {
		if (data && data.email === req.body.email)
          ...
          activity(req.body.email+' module. '+ms.messages[3].message, req.headers["user"], req.socket.remoteAddress.split(":").pop(), 'admin', req.session.useragent, req.session.useragent.create);
...
exports.findAll = async(req, res) => {
  ...
  var condition = { $or: [{ email: { $regex: new RegExp(search), $options: "i" }}, { abbreviation: { $regex: new RegExp(search), $options: "i" }}, { modifiedBy: { $in: info1 } }, { createdBy: { $in: info1 } } ]};
  }
  else
  condition = {};
...
exports.findStates = async (req, res) => {
	...
	Table.find({ status: "Active" }).sort({ email: 1 })
	  .then((data) => {
		var info = [];
		data.forEach(function (doc, err) {
		  info.push({ label: doc.email, value: doc._id });
		});
		res.send({ list: info });
	  })
	  .catch((err) => {
