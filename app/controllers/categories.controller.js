...
exports.findAll = async (req, res) => {
  ...
  const { page, size, search, field, dir, status, state, show, type } = req.query;
  var sortObject = {};
  ...
  Table.paginate(condition, { collation: { locale: "en" }, populate: [{ path: "createdBy", select: ["firstname", "lastname"] }, { path: "modifiedBy", select: ["firstname", "lastname"] },], offset, limit, sort: sortObject })
    .then((data) => {
      res.send({
...
exports.findList = async (req, res) => {
  ...
  Table.find({ status: "Active", type: type }).sort({ name: 1 })
    .then((data) => {
    res.send({ list: data });
    })
    .catch((err) => {
    res.status(400).send({ message:"Can't Find Categories" });
    });
